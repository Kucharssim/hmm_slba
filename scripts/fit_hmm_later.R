...
data <- readr::read_csv(here::here("data", "dutilh_2010_subject_A.csv"))
...
p_cum_check <- cum_check %>%
  ...
  ggplot(aes(x = rt, y = probability/nrow(cum_check), group = response, col = response)) +
  geom_line(size = 1) +
  facet_wrap(.~state) +
  #ylim(0:1) + 
...
for(i in seq_len(nrow(cum_check_2))) {
  cum_check_2$cdf[i] <- plater(rt = cum_check_2$rt[i], response = cum_check_2$response[i], 
...
cum_check_2 %>%
  mutate(state = sprintf("State %s", state), response = as.factor(response)) %>%
  ggplot(aes(x = rt, y = cdf, group = response, col = response)) +
  geom_line(size = 1) +
  facet_wrap(.~state) +
  #ylim(0:1) + 
  ylab("Cumulative probability") + xlab("Time (sec)") +
